...
还有一个原因是 `Windows Defender`引起的，将如下两个设置关闭：
![](../../attachments/Pasted%20image%2020251111172703.png)
...
如果发现是`nexus`缓存，某个包找不到对应的版本，比如`http://188.2.27.132/repository/nuget-group/v3/content/microsoft.extensions.diagnostics/index.json`，然后看官方的地址 `https://api.nuget.org/v3-flatcontainer/microsoft.extensions.diagnostics/index.json` 是不是确实有那个版本。如果确认是缓存问题，需要用管理员账户到那个仓库代理中点击 `Invalidate Cache`
