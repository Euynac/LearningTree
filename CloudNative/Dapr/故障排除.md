...
### K8S无法通过服务NodePort访问工作负载
如果daprd出现问题，会导致 Pod Ready = False → Service Endpoints 为空，导致无法路由流量。自然也无法进入到swagger。
需要排查问题可以临时修改Service：publishNotReadyAddresses：true，强行路由到未就绪的负载。