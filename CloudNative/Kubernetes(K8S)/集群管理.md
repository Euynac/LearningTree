...
其中`RADOS`全称为Reliable Autonomic Distributed `Object` Store, 可靠的、自动化的、分布式对象存储系统。
- 优点：
...
容器下面`ba`sh``、`sh`可以用，还可以通过文件浏览器直接管理容器的文件。

快捷访问端口：开通的端口可以简单通过端口右边绿色的按钮通过代理访问。

# Minikube

1 `Node` K8s `Cluster`，即Master node、Work node都在一个`Node`上。
